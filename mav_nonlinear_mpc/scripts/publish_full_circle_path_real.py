...
class Publish_Path(object):
    def __init__(self, mav_name, uav_num, circle_r, altitude, vel_mag, publish_interval, ref_time_step):
    ...
    def write_callback(self, msg):
        t_read = msg.data.secs + msg.data.nsecs * 1e-9 # t_read in seconds
        #print(t_read)
        if self.t0 == -1:
            self.t0 = t_read
        t_since_start = t_read - self.t0
        if t_since_start < 0:
            self.t0 = t_read
            t_since_start = 0.0
        d = self.circle_r
